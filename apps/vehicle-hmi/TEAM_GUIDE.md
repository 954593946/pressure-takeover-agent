...
- 底部：左侧唯一确认按钮、中央语音提示、右侧空调只读摘要和 Agent 配置。

消息草稿、行程详情、方案详情、车况详情和三端同步通过主驾驶侧浮层面板查看；浮层不得使用整屏灰色遮罩，导航和确认入口需要保持上下文可见。不得把控制台日志、商品长列表或消息全文铺在主屏上。
...
`方案`、`车况`、`同步`、`消息草稿`、`行程详情`均为二级信息入口，只读展示当前 `WorldState` 摘要，不直接改写状态。二级信息以局部浮层承载，保持地图主任务可见，不做网页式全屏遮罩。

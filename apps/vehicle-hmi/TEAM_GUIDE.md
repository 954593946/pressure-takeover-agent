...
导航页提供“3D 跟车”和“路线全览”两种驾驶视角。跟车态应看到真实高德底图、路线向前、自车固定在下部，路线当前位置投影与固定车标误差不得超过 4px；全览态应看到整条路线和起终点。若设备没有可用 WebGL，页面保持真实高德 2D 全览并禁用跟车，不会用旋转 DOM 或 Bosch 假地图伪装 3D。正式展示设备必须提前运行 `e2e_live_amap_navigation.py` 或人工核对跟车、拥堵停车、恢复行驶和全览切换。

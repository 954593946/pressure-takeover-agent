...
P0 区域：路线/ETA、Agent 状态、现实结论、动作列表和“确认发送”大按钮。驾驶中不做长文本、多选决策、真实地图 SDK 的完整导航能力、多轮聊天或真实车控。

驾驶主屏采用 3:7 布局：左侧是驾驶员近侧 AURI 面板，右侧是持续可见的导航主画布。导航画布使用车道级放大图、立体街区、道路分级、POI、路况分段和路线进度表达 Demo 所需导航语义；它是可控的 HMI 模拟层，不是高德地图 SDK。主屏只保留一句判断、一个动作组、一个主要确认入口、ETA 和刚性/弹性任务摘要；速度、限速和档位放在地图右上角。消息草稿、方案、行程、车况和三端同步通过驾驶员侧原位二级页查看，不遮罩或压暗地图。

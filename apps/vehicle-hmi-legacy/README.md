...
驾驶主屏采用 3:7 布局：左侧是驾驶员近侧 AURI 面板，右侧是持续可见的导航主画布。页面启动时默认从 Agent `/v1/map-config` 获取高德 Web JS API 配置，加载高德真实 3D 底图、道路和 POI、驾车路线、实时交通图层及车辆沿线动画；Key 缺失、网络失败或额度保护触发时才回退 SVG 离线地图。驾驶阶段采用路径朝向驱动的 3D 跟车相机，路线预览和停车阶段回到全览视角，用户也可使用地图上的“全览 / 跟车”分段控件手动切换。两种地图共用同一套 Agent 状态覆盖层，灰色表示已行驶、蓝色表示剩余路线，黄色表示拥堵风险。主屏只保留一句判断、一个动作组、一个主要确认入口、ETA 和刚性/弹性任务摘要；速度、限速和档位放在地图右上角。消息草稿、方案、行程、车况和三端同步通过驾驶员侧原位二级页查看，不遮罩或压暗地图。

页面默认展示“等待手机语音创建任务”，不注入任何预置任务。演示控制台可选择载入预置故事；其拥堵事件按当前刚性任务的时间计算 `eta`。车机所有到达时间和晚到判断以 Agent World State 为准。腕上状态为 `warning` 或 `handover` 且包含触觉指令时，车机展示可手动关闭的低干扰提示卡。

在线路线起点为公开办公地点“博世苏州・星龙街455号”，终点“阳光小学”为 Demo 冻结坐标，不代表任何真实儿童学校。路线进度按真实累计距离计算，不按高德返回的坐标点数量计算；车辆位置、已行驶路线、剩余路线和前方拥堵段使用同一套距离进度。高德路线仅在 HMI 页面初始化时规划一次，World State、SSE、轮询和 Demo 阶段变化只更新现有覆盖物。浏览器本地设置每月 200 次地图初始化和 200 次路线规划的保守保护阈值，达到后自动回退离线地图。

底部任务摘要使用“责任类型 + 任务 + 状态标签”，刚性责任和弹性任务不会只靠颜色区分。两项以内直接显示全部任务；超过两项时主屏显示首要任务和其余任务摘要，完整列表进入“方案”二级页，避免驾驶主屏横向滚动和卡片截断。行程摘要使用“大数字 + 小单位”展示剩余距离、预计用时和到达时间，1280×720 下不得截断关键值。
...
地图配置同样来自 Agent，但与 World State 职责分离：

```text
GET /v1/map-config
-> 返回高德公开 Web JS Key、服务端代理地址和地图样式
-> HMI 初始化真实 3D 地图和一次驾车路线规划
-> Security JS Code 始终保留在 Agent 服务端
```
...
任务区不假设固定任务数量。导航下方任务卡、方案二级页、联系人消息和动作进度都直接读取当前 `WorldState.tasks` 与 `WorldState.actions`；超过两项时主屏压缩为“首要任务 + 其余任务摘要”，二级页保留全部任务。座舱空调只读取 `WorldState.vehicle_state`，字段缺失时显示“等待 Agent 同步”，不会伪造默认温度。空调控制回复只进入座舱状态，不覆盖驾驶主界面的现实结论。

## 主驾驶侧 Agent 交互

左侧 Agent 面板提供驾驶中低干扰的状态入口：

- `手机语音同步`：只读显示 `WorldState.last_utterance`。语音输入和 ASR 位于手机端，车机不提供麦克风入口。
...
二级页采用与座舱状态一致的“状态总览 + 分组卡片”设计：

- `任务`：顶部突出当日关键责任，刚性与弹性任务使用可展开卡片，处理进度单独呈现。
- `消息`：按 `actions[]` 中实际联系人分段，可显示老师、孩子妈妈、爷爷或奶奶，展示草稿状态和“生成草稿 → 等待确认/已发送”进度。
- `同步`：用手机、腕上、车机三端流转图和设备状态卡标出当前主交互端、腕上触觉和 revision。

消息长文本必须自然换行，完成摘要会按消息结果、订单数量金额和配送时间拆段，禁止用单行省略号隐藏关键事实。后端触觉枚举必须转换为“双短震”“三拍提示”等用户可理解文案，不直接展示 `three_beat` 等内部字段。

主判断区同样使用状态卡而不是纯文字堆叠：阶段图标、阶段名称、当前主端和处理阶段位于判断卡；现实结论单独成卡。`parked_review` 阶段显示“手机接续”，提供“消息记录”和“处理结果”两个只读复盘入口，并隐藏已经失去意义的座舱状态入口。

顶部接送指标禁止使用“责任窗口 / 突破 / 压缩”等内部概念，统一显示用户结果：

```text
接送任务 / 晚到 18 分钟 / 原定 18:10
接送任务 / 出发时间紧张 / 原定 18:10
接送任务 / 可按时到达 / 原定 18:10
```
